...
public class OpenbankDAO {
	...
	public List<OpenbankVO> nsAcList(OpenbankVO vo) {
		...
		String query = " SELECT c.accnum, c.id, ui.NAME, c.AC_NAME, c.AC_MONEY, c.AC_OP_DATE, c.AC_ED_DATE, c.bank_cd, c.bank_name, d.pd_name "
				+ " FROM ("
				+ " SELECT a.accnum, a.id, a.AC_NAME, a.AC_MONEY, a.AC_OP_DATE, a.AC_ED_DATE, a.PD_NUMBER, a.bank_cd, b.bank_name "
				+ " FROM account@connectSH a " + " JOIN bankinfo@connectSH b ON b.bank_cd = a.bank_cd " + " ) c "
				+ " JOIN product@connectSH d ON d.PD_NUMBER = c.PD_NUMBER "
				+ " JOIN USER_INFO@connectSH ui ON ui.ID = c.id " + " WHERE c.id = ? ORDER BY c.AC_OP_DATE DESC ";
	...
	public List<OpenbankVO> bhAcList(OpenbankVO vo) {
		List<OpenbankVO> accountList = new ArrayList<OpenbankVO>();
		String query = "SELECT c.Ac_number, c.id, ui.name, c.AC_NAME, c.AC_MONEY, c.AC_OP_DATE, c.AC_ED_DATE, c.bank_cd, c.bank_name "
				+ "FROM " + "( "
				+ "    SELECT a.Ac_number, a.id, a.AC_NAME, a.AC_MONEY, a.AC_OP_DATE, a.AC_ED_DATE, a.PD_NUMBER, a.bank_cd, b.bank_name "
				+ "    FROM account@connectBH a " + "    JOIN bankinfo@connectBH b ON b.bank_cd = a.bank_cd " + ") c "
				+ "JOIN USER_INFO@connectBH ui ON ui.ID = c.id " + "WHERE c.id = ? " + "ORDER BY c.AC_OP_DATE DESC";
	...
	public List<OpenbankVO> plAcList(OpenbankVO vo) {
		...
		String query = "SELECT c.Ac_number, c.id, ui.NAME, c.AC_NAME, c.AC_MONEY, c.AC_OP_DATE, c.AC_ED_DATE, c.bank_cd, c.bank_name, d.pd_name "
				+ "FROM " + "( "
				+ "   SELECT a.Ac_number, a.id, a.AC_NAME, a.AC_MONEY, a.AC_OP_DATE, a.AC_ED_DATE, a.PD_NUMBER, a.bank_cd, b.bank_name "
				+ "   FROM account@connectJY a " + "   JOIN bankinfo@connectJY b ON b.bank_cd = a.bank_cd " + ") c "
				+ "JOIN product@connectJY d ON d.PD_NUMBER = c.PD_NUMBER "
				+ "JOIN USER_INFO@connectJY ui ON ui.ID = c.id " + "WHERE c.id = ? " + "ORDER BY c.AC_OP_DATE DESC";
	...
	public OpenbankVO getAccount(long ac_number, String bank_cd) {
		...
		case "204":
			query = " SELECT c.ac_number, c.id, ui.NAME, c.AC_NAME, c.AC_MONEY, c.AC_OP_DATE, c.AC_ED_DATE, c.bank_cd, c.bank_name, d.pd_name "
					...
					+ " FROM account@connectJY a " + " JOIN bankinfo@connectJY b ON b.bank_cd = a.bank_cd " + " ) c "
					+ " JOIN product@connectJY d ON d.PD_NUMBER = c.PD_NUMBER "
					+ " JOIN USER_INFO@connectJY ui ON ui.ID = c.id " + " where c.ac_number = ? and c.bank_cd = ? ";
			break;
		case "159":
			query = " SELECT c.accnum, c.id, ui.NAME, c.AC_NAME, c.AC_MONEY, c.AC_OP_DATE, c.AC_ED_DATE, c.bank_cd, c.bank_name, d.pd_name "
					...
					+ " FROM account@connectSH a " + " JOIN bankinfo@connectSH b ON b.bank_cd = a.bank_cd " + " ) c "
					+ " JOIN product@connectSH d ON d.PD_NUMBER = c.PD_NUMBER "
					+ " JOIN USER_INFO@connectSH ui ON ui.ID = c.id " + " where c.accnum = ? and c.bank_cd = ? ";
			break;
		case "111":
			query = " SELECT c.ac_number, c.id, ui.NAME, c.AC_NAME, c.AC_MONEY, c.AC_OP_DATE, c.AC_ED_DATE, c.bank_cd, c.bank_name "
					+ " FROM ("
					+ " SELECT a.ac_number, a.id, a.AC_NAME, a.AC_MONEY, a.AC_OP_DATE, a.AC_ED_DATE, a.PD_NUMBER, a.bank_cd, b.bank_name "
					+ " FROM account@connectBH a " + " JOIN bankinfo@connectBH b ON b.bank_cd = a.bank_cd " + " ) c "
					+ " JOIN USER_INFO@connectBH ui ON ui.ID = c.id " + " where c.ac_number = ? and c.bank_cd = ? ";
//						+ " JOIN product@bhBank d ON d.PD_NUMBER = c.PD_NUMBER "
		...
		case "616":
			query = " SELECT c.ac_number, c.id, ui.NAME, c.AC_NAME, c.AC_MONEY, c.AC_OP_DATE, c.AC_ED_DATE, c.bank_cd, c.bank_name, d.pd_name "
					+ " FROM ("
					+ " SELECT a.ac_number, a.id, a.AC_NAME, a.AC_MONEY, a.AC_OP_DATE, a.AC_ED_DATE, a.PD_NUMBER, a.bank_cd, b.bank_name "
					+ " FROM account a " + " JOIN bank_info b ON b.bank_cd = a.bank_cd " + " ) c "
					+ " JOIN products d ON d.PD_NUMBER = c.PD_NUMBER " + " JOIN USER_INFO ui ON ui.ID = c.id "
	...
	// 이체
	public int transfer(long sendAc_number, String sendBank_cd, long receivAc_number, String receivBank_cd,
		...
		switch (sendBank_cd) {
		case "204":
			withdrawalQuery = " UPDATE account@connectJY SET AC_MONEY = AC_MONEY - ? "
					+ " WHERE ac_number = ? AND bank_cd = ?";
			break;
		case "159":
			withdrawalQuery = " UPDATE account@connectSH SET AC_MONEY = AC_MONEY - ? "
					+ " WHERE accnum = ? AND bank_cd = ? ";
			break;
		case "111":
			withdrawalQuery = " UPDATE account@connectBH SET AC_MONEY = AC_MONEY - ? "
					+ " WHERE ac_number = ? AND bank_cd = ? ";
			break;
		case "616":
			withdrawalQuery = " UPDATE account SET AC_MONEY = AC_MONEY - ?" + " WHERE ac_number = ? AND bank_cd = ? ";
			break;
		default:
			// 다른 은행 코드에 대한 처리를 여기에 추가하거나 또는 에러 처리를 할 수 있습니다.
			throw new RuntimeException("지원되지 않는 은행 코드입니다.");
		...
		case "204":
			depositQuery = "UPDATE account@connectJY SET AC_MONEY = AC_MONEY + ? WHERE ac_number = ? AND bank_cd = ?";
			break;
		case "159":
			depositQuery = "UPDATE account@connectSH SET AC_MONEY = AC_MONEY + ? WHERE accnum = ? AND bank_cd = ?";
			break;
		case "111":
			depositQuery = "UPDATE account@connectBH SET AC_MONEY = AC_MONEY + ? WHERE ac_number = ? AND bank_cd = ?";
			break;
		case "616":
		...
		try {
			...
			if (withdrawalResult > 0 && depositResult > 0) {
				...
				RecordDAO ac_recordDAO = new RecordDAO();

				// 출금 이력 추가
				ac_recordDAO.insertTransaction(sendAc_number, sendBank_cd, "출금", receivBank_cd, receivAc_number,
						transferAmount, rc_text);

				// 입금 이력 추가
				ac_recordDAO.insertTransaction(receivAc_number, receivBank_cd, "입금", sendBank_cd, sendAc_number,
						transferAmount, rc_text);
